...
extern "C"
{
    ...
    __global__ void annealing_module(int mean_range, int vertex, float mem_I0, int8_t *h_vector, int8_t *J_matrix, int *spin_vector, float *rnd, float *D_res, float *lambda, float *delta, int *nu, int count_device)
    {
        ...
        if (i < vertex)
        {
            if (count_device % nu[i] == 0)
            {
                ...
                //for(b=0; b<tau; b++){
                    ...
                    Itanh = tanh(lambda[i] * mem_I0 * (avg_D_res + delta[i]))  + rnd[i];
